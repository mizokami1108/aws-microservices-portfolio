...
export interface OrderServiceConstructProps {
  cluster: ecs.Cluster;
...
export class OrderServiceConstruct extends Construct {
  public readonly service: ecs.FargateService;
  public readonly loadBalancer: elbv2.ApplicationLoadBalancer;

  constructor(scope: Construct, id: string, props: OrderServiceConstructProps) {
    super(scope, id);

    // 設定ファイルから設定を読み込み
    const serviceConfig = ConfigLoader.getServiceConfig("orderService");
    ...
    const container = taskDefinition.addContainer("Container", {
      ...
      environment: {
        DDB_TABLE: props.orderTable.tableName,
      },
    });
    ...
    // DynamoDBへのアクセス権限を付与
    props.orderTable.grantReadWriteData(taskDefinition.taskRole);

    // 設定の表示（デバッグ用）
    console.log(`Order Service - Desired Count: ${serviceConfig.desiredCount}`);
  }
}
